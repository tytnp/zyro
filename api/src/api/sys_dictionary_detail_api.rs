...
) -> Json<sys_dictionary_detail::Model> {
...
pub async fn del(
    ...
    if let Ok(Some(res)) = SysDictionaryDetail::find_by_id(model.id)
        .one(&state.conn)
        .await
    {
        SysDictionaryDetail::delete_by_id(res.id)
            .exec(&state.conn)
            .await
            .unwrap();
        return Json(res);
...
pub async fn edit(
    ...
    let mut active_model = SysDictionaryDetail::find_by_id(model.id)
        .one(&state.conn)
        .await
        .unwrap()
        .unwrap()
...
pub async fn one(
    ...
    Json(
        SysDictionaryDetail::find_by_id(model.id)
            .one(&state.conn)
            .await
            .unwrap()
            .unwrap(),
