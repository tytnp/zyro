...
pub async fn add(
    ...
    Json(model): Json<sys_api::Model>,
) -> Json<ApiRet<sys_api::Model>> {
    ...
    let result = match active_model.insert(&state.conn).await {
        Ok(m) => ApiRet::<sys_api::Model>::with_data(m),
        Err(err) => ApiRet::<sys_api::Model>::with_error(Error::from(err)),
    };
    Json(result)
}
pub async fn del(
    ...
    let result = match SysApi::find_by_id(model.id).one(&state.conn).await {
        Ok(Some(res)) => match SysApi::delete_by_id(model.id).exec(&state.conn).await {
            Ok(_) => ApiRet::<sys_api::Model>::with_data(res),
            Err(err) => ApiRet::<sys_api::Model>::with_error(Error::from(err)),
        },
        Err(err) => ApiRet::<sys_api::Model>::with_error(Error::from(err)),
        _ => ApiRet::<sys_api::Model>::new(),
    };
    return Json(result);
}
pub async fn edit(
...
pub async fn one(
    ...
    let result = match SysApi::find_by_id(model.id).one(&state.conn).await {
        Ok(Some(m)) => ApiRet::<sys_api::Model>::with_data(m),
        Err(err) => ApiRet::<sys_api::Model>::with_error(Error::from(err)),
        _ => ApiRet::<sys_api::Model>::new(),
    };
    Json(result)
