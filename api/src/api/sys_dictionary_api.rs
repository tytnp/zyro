...
pub async fn del(
    ...
    if let Ok(Some(res)) = SysDictionary::find_by_id(model.id).one(&state.conn).await {
        SysDictionary::delete_by_id(res.id)
            .exec(&state.conn)
            .await
            .unwrap();
        return Json(res);
...
pub async fn edit(
    ...
    let mut active_model = SysDictionary::find_by_id(model.id)
        .one(&state.conn)
        .await
        .unwrap()
        .unwrap()
...
pub async fn one(
    ...
    Json(
        SysDictionary::find_by_id(model.id)
            .one(&state.conn)
            .await
            .unwrap()
            .unwrap(),
